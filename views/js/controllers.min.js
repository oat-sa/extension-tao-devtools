...
$("#Extensions_manager table tr:nth-child(odd)").addClass("extensionOdd")}var ext_installed=[],toInstall=[],indexCurrentToInstall=-1,percentByExt=0,installError=0;return{start:function(){styleTables(),$("#installProgress").hide(),$("#addButton").click(function(e){e.preventDefault(),section.create({id:"devtools-newextension",name:__("Create new extension"),url:helpers._url("create","ExtensionsManager","taoDevTools",{uri:$("#openuri").val()})}).show()}),$("#extensions-manager-container .ext-id").each(function(){var ext=$(this).text();ext_installed.push(ext),$(".ext-id.ext-"+ext).addClass("installed")}),$("#available-extensions-container tr input").click(function(event){
event.stopPropagation()}),$("#available-extensions-container tr input:checkbox").click(function(){var $installButton=$("#installButton");$(this).parent().parent().parent().find("input:checkbox:checked").length>0?$installButton.attr("disabled",!1):$installButton.attr("disabled",!0)}),$("#installButton").click(function(event){return toInstall=[],$("#available-extensions-container input:checked").each(function(){var ext=$(this).prop("name").split("_")[1],deps=getDependencies(ext);deps.length&&(toInstall=toInstall.concat(deps)),toInstall.push(ext)}),toInstall=getUnique(toInstall),0==toInstall.length?(alert(__("Nothing to install !")),!1):(percentByExt=100/toInstall.length,
$("#installProgress p.status").text(__("%s extension(s) to install.").replace("%s",toInstall.length)),$("#installProgress .bar").width(0),$("#installProgress .console").empty(),progressConsole(__("Do you wish to install the following extension(s):\n%s?").replace("%s",toInstall.join(", "))),$("#installProgress").dialog({modal:!0,width:400,height:300,buttons:[{text:__("No"),click:function(){$(this).dialog("close")}},{text:__("Yes"),click:function(){progressConsole(__("Preparing installation...")),$(".ui-dialog-buttonpane").remove(),installError=0,indexCurrentToInstall=0,installNextExtension()}}]}),void event.preventDefault())}),$(".disableButton").click(function(event){
var id=$(event.target).data("extid");$.ajax({type:"POST",url:helpers._url("disable","ExtensionsManager","taoDevTools"),data:"id="+id,dataType:"json",success:function(data){data.success?(feedback().success(data.message),setTimeout(function(){window.location.reload()},1e3)):feedback().info(data.message)}})}),$(".enableButton").click(function(event){var id=$(event.target).data("extid");$.ajax({type:"POST",url:helpers._url("enable","ExtensionsManager","taoDevTools"),data:"id="+id,dataType:"json",success:function(data){data.success?(feedback().success(data.message),setTimeout(function(){window.location.reload()},1e3)):feedback().info(data.message)}})}),$(".uninstallButton").click(function(event){
var id=$(event.target).data("extid");$.ajax({type:"POST",url:helpers._url("uninstall","ExtensionsManager","taoDevTools"),data:"id="+id,dataType:"json",success:function(data){data.success?(feedback().success(data.message),setTimeout(function(){window.location.reload()},1e3)):feedback().info(data.message)}})})}}});
