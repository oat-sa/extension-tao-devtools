...
currentShortcut=null,shortcutsResults=[],setResults(),actions.nextShortcut()},previousShortcut:function(){removeShortcut(),function(){for(currentShortcut=null;!currentShortcut;)if((currentShortcut=listOfKeys[--current])&&(currentShortcut.platform&&"all"!==currentShortcut.platform&&currentShortcut.platform.indexOf(platformType)<0?currentShortcut=null:currentShortcut.browser&&"all"!==currentShortcut.browser&&currentShortcut.browser.indexOf(browserType)<0&&(currentShortcut=null)),current<0){current=0,currentShortcut=null;break}}(),updateKeyChecker()},nextShortcut:function(){removeShortcut(),function(){for(currentShortcut=null;!currentShortcut;)if((
currentShortcut=listOfKeys[++current])&&(currentShortcut.platform&&"all"!==currentShortcut.platform&&currentShortcut.platform.indexOf(platformType)<0?currentShortcut=null:currentShortcut.browser&&"all"!==currentShortcut.browser&&currentShortcut.browser.indexOf(browserType)<0&&(currentShortcut=null)),current>=listOfKeys.length){currentShortcut=null;break}return currentShortcut}()?updateKeyChecker():(hider.hide($container.find(".desk")),hider.show($container.find(".end")),actions.showResults())}};function setResults(){function done(){resultData.shortcuts=_.compact(shortcutsResults),$container.find(".results pre").html(JSON.stringify(resultData,null,2))}
