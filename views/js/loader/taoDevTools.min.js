...
function($,__,urlUtil,section,feedback){var ext_installed=[],toInstall=[],indexCurrentToInstall=-1,percentByExt=0,installError=0;function progressConsole(msg){$("#installProgress .console").append("<p>"+msg+"</p>"),$("#installProgress .console").prop({scrollTop:$("#installProgress .console").prop("scrollHeight")})}function installNextExtension(){var ext=toInstall[indexCurrentToInstall];$("#installProgress p.status").text(__("Installing extension %s...").replace("%s",ext)),progressConsole(__("Installing extension %s...").replace("%s",ext)),$.ajax({type:"POST",url:urlUtil.route("install","ExtensionsManager","tao"),data:"id="+ext,dataType:"json",
success:function(data){data.success?(progressConsole(__("> Extension %s succesfully installed.").replace("%s",ext)),$("#installProgress .bar").animate({width:"+="+percentByExt+"%"},1e3,function(){++indexCurrentToInstall>=toInstall.length?(toInstall=[],$("#installProgress .bar").animate({backgroundColor:"#bb6",width:"100%"},1e3),progressConsole(__("Generating cache...")),$.ajax({type:"GET",url:$($("#main-menu a")[0]).prop("href"),success:function(data){$("#installProgress .bar").animate({backgroundColor:"#6b6"},1e3),$("#installProgress p.status").text(__("Installation done.")),progressConsole(__("> Installation done.")),window.location.reload()}})
):installNextExtension()})):(installError=1,progressConsole("Installation of "+ext+" failed")),feedback().info(data.message)}}),installError&&progressConsole(__("A fatal error occured during the installation process."))}return{start:function(){$("#Extensions_manager table tr").removeClass("extensionOdd").removeClass("extensionEven"),$("#Extensions_manager table tr:nth-child(even)").addClass("extensionEven"),$("#Extensions_manager table tr:nth-child(odd)").addClass("extensionOdd"),$("#installProgress").hide(),$("#addButton").click(function(e){e.preventDefault(),section.create({id:"devtools-newextension",name:__("Create new extension"),url:urlUtil.route(
"create","ExtensionsManager","taoDevTools"),contentBlock:!0}).show()}),$("#extensions-manager-container .ext-id").each(function(){var ext=$(this).text();ext_installed.push(ext),$(".ext-id.ext-"+ext).addClass("installed")}),$("#available-extensions-container tr input").click(function(event){event.stopPropagation()}),$("#available-extensions-container tr input:checkbox").click(function(){var $installButton=$("#installButton");0<$(this).parent().parent().parent().find("input:checkbox:checked").length?$installButton.attr("disabled",!1):$installButton.attr("disabled",!0)}),$("#installButton").click(function(event){var $modalContainer=$("#installProgress");if(
event.preventDefault(),toInstall=[],$("#available-extensions-container input:checked").each(function(){var ext=$(this).prop("name").split("_")[1],deps=function getDependencies(extension){var dependencies=[];return $("#"+extension+" .dependencies li:not(.installed)").each(function(){var ext=$(this).attr("rel"),deps=getDependencies(ext);deps.push(ext),dependencies=dependencies.concat(deps)}),dependencies}(ext);(toInstall=deps.length?toInstall.concat(deps):toInstall).push(ext)}),0==(toInstall=function(orig){for(var a=[],i=0;i<orig.length;i++)$.inArray(orig[i],a)<0&&a.push(orig[i]);return a}(toInstall)).length)return window.alert(__("Nothing to install !")),!1;
percentByExt=100/toInstall.length,$("p.status",$modalContainer).text(__("%s extension(s) to install.").replace("%s",toInstall.length)),$(".bar",$modalContainer).width(0),$(".console",$modalContainer).empty(),progressConsole(__("Do you wish to install the following extension(s):\n%s?").replace("%s",toInstall.join(", "))),$("[data-control=cancel]",$modalContainer).on("click",function(e){e.preventDefault(),$modalContainer.modal("close")}),$("[data-control=confirm]",$modalContainer).on("click",function(e){e.preventDefault(),progressConsole(__("Preparing installation...")),$(".buttons",$modalContainer).remove(),indexCurrentToInstall=installError=0,
installNextExtension()}),$modalContainer.modal({width:400,height:300,top:150,disableEscape:!0,disableClosing:!0})}),$(".disableButton").click(function(event){event=$(event.target).data("extid");$.ajax({type:"POST",url:urlUtil.route("disable","ExtensionsManager","taoDevTools"),data:"id="+event,dataType:"json",success:function(data){data.success?(feedback().success(data.message),setTimeout(function(){window.location.reload()},1e3)):feedback().info(data.message)}})}),$(".enableButton").click(function(event){event=$(event.target).data("extid");$.ajax({type:"POST",url:urlUtil.route("enable","ExtensionsManager","taoDevTools"),data:"id="+event,dataType:"json",
success:function(data){data.success?(feedback().success(data.message),setTimeout(function(){window.location.reload()},1e3)):feedback().info(data.message)}})}),$(".uninstallButton").click(function(event){event=$(event.target).data("extid");$.ajax({type:"POST",url:urlUtil.route("uninstall","ExtensionsManager","taoDevTools"),data:"id="+event,dataType:"json",success:function(data){data.success?(feedback().success(data.message),setTimeout(function(){window.location.reload()},1e3)):feedback().info(data.message)}})})}}}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),
