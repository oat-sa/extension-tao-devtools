define("taoDevTools/controller/ControllerMap/index",function(){}),define("taoDevTools/controller/fontConversion/fileUpload",["jquery","uikitLoader","ui/uploader","ui/feedback"],function($,ui,uploader,feedback){$=$("#upload-container");$.uploader({uploadUrl:$.data("url")}),$.on("upload.uploader",function(e,file,interactionHook){feedback().success(interactionHook.success)}),$.on("fail.uploader",function(e,file,interactionHook){feedback().error(interactionHook.message)})}),define("json!taoDevTools/controller/KeyChecker/shortcuts.json",function(){return[{label:"<Meta>+C",shortcut:"Meta+C",platform:"mac",browser:"all",description:"Copy to clipboard.",
playground:!0},{label:"<Meta>+V",shortcut:"Meta+V",platform:"mac",browser:"all",description:"Paste from clipboard.",playground:!0},{label:"<Meta>+<Alt>+V",shortcut:"Meta+Alt+V",platform:"mac",browser:"all",description:"Paste Style",playground:!0},{label:"<Meta>+<Shift>+<Alt>+V",shortcut:"Meta+Shift+Alt+V",platform:"mac",browser:"all",description:"Paste and Match Style",playground:!0},{label:"<Meta>+X",shortcut:"Meta+X",platform:"mac",browser:"all",description:"Cut to the clipboard",playground:!0},{label:"<Meta>+:",shortcut:"Meta+:",platform:"mac",browser:"all",description:"Show / Hide Spelling and Grammar",playground:!1},{label:"<Ctrl>+F3",
shortcut:"Ctrl+F3",platform:"mac",browser:"all",description:"Opens windows menu / dock.",playground:!1},{label:"<Ctrl>+F2",shortcut:"Ctrl+F2",platform:"mac",browser:"all",description:"Activate Menu",playground:!1},{label:"<Ctrl>+F8",shortcut:"Ctrl+F8",platform:"mac",browser:"all",description:"Active Menu Extras",playground:!1},{label:"<Shift>+<Meta>+Right",shortcut:"Shift+Meta+Right",platform:"mac",browser:"all",description:"Switch Applications",playground:!1},{label:"<Meta>+<Alt>+W",shortcut:"Meta+Alt+W",platform:"mac",browser:"all",description:"Close application",playground:!1},{label:"<Meta>+<Shift>+3",shortcut:"Meta+Shift+3",platform:"mac",browser:"all",
description:"Take screenshot (entire screen)",playground:!1},{label:"<Meta>+<Shift>+4",shortcut:"Meta+Shift+4",platform:"mac",browser:"all",description:"Take screenshot (partial / active window)",playground:!1},{label:"<Meta>+<Shift>+W",shortcut:"Meta+Shift+W",platform:"mac",browser:"all",description:"Closes the current window.",playground:!1},{label:"<Meta>+M",shortcut:"Meta+M",platform:"mac",browser:"all",description:"Minimizes the window.",playground:!1},{label:"<Meta>+<Alt>+M",shortcut:"Meta+Alt+M",platform:"mac",browser:"all",description:"Minimize (All)",playground:!1},{label:"backspace",shortcut:"backspace",platform:"mac",browser:"chrome",
...
currentShortcut=null,shortcutsResults=[],setResults(),actions.nextShortcut()},previousShortcut:function(){for(removeShortcut(),currentShortcut=null;!currentShortcut;)if((currentShortcut=listOfKeys[--current])&&(currentShortcut.platform&&"all"!==currentShortcut.platform&&currentShortcut.platform.indexOf(platformType)<0||currentShortcut.browser&&"all"!==currentShortcut.browser&&currentShortcut.browser.indexOf(browserType)<0)&&(currentShortcut=null),current<0){current=0,currentShortcut=null;break}updateKeyChecker()},nextShortcut:function(){removeShortcut(),!function(){currentShortcut=null;for(;!currentShortcut;)if((currentShortcut=listOfKeys[++current])&&(
currentShortcut.platform&&"all"!==currentShortcut.platform&&currentShortcut.platform.indexOf(platformType)<0||currentShortcut.browser&&"all"!==currentShortcut.browser&&currentShortcut.browser.indexOf(browserType)<0)&&(currentShortcut=null),current>=listOfKeys.length){currentShortcut=null;break}return currentShortcut}()?(hider.hide($container.find(".desk")),hider.show($container.find(".end")),actions.showResults()):updateKeyChecker()}};function setResults(){function done(){resultData.shortcuts=_.compact(shortcutsResults),$container.find(".results pre").html(JSON.stringify(resultData,null,2))}keycheckStorage.setItem("state",{current:current,
results:shortcutsResults}).then(done).catch(done)}function updateCurrentResult(){var defaultPrevented=$container.find('input[name="default_prevented"][value="1"]').is(":checked"),comment=$container.find(".comment textarea").val();currentShortcut&&(shortcutsResults[current]={shortcut:formatShortcut(currentShortcut.label),shortcutCaught:shortcutCaught,defaultPrevented:defaultPrevented},$container.find('.prevented label[for="default_prevented_yes"]').css("color",defaultPrevented?"green":null),$container.find('.prevented label[for="default_prevented_no"]').css("color",defaultPrevented?null:"red"),comment?shortcutsResults[current
].comment=comment:delete shortcutsResults[current].comment,setResults())}function updateKeyChecker(){$container.find('.prevented input[value="0"]').click(),$container.find(".caught .value").html("No").css("color","red"),$container.find(".comment textarea").val(""),shortcutCaught=!1,currentShortcut&&(shortcutHelper.add(currentShortcut.shortcut,function(event,keystroke){console.log("caught keystroke",keystroke),shortcutCaught=!0,$container.find(".caught .value").html("Yes").css("color","green"),hider.show($container.find(".prevented")),updateCurrentResult()}),$container.find(".key .value").html(formatShortcut(currentShortcut.label)),$container.find(
".description .value").html(formatShortcut(currentShortcut.description)),$container.find(".playground textarea").val("The quick brown fox jumps over the lazy dog"),$container.find(".playground").toggle(!!currentShortcut.playground)),updateCurrentResult(),hider.hide($container.find(".prevented")),hider.hide($container.find(".end")),hider.show($container.find(".desk"))}function removeShortcut(){currentShortcut&&shortcutHelper.remove(currentShortcut.shortcut)}function formatShortcut(label){return _.forEach(specialKeys[platformType],function(spec,code){label=label.replace(code,spec)}),label}function displaySection(name){hider.hide($container.find(".section")),
hider.show($container.find(".section."+name))}$(document).on("click","button",function(e){e=$(e.target).data("control");actions[e]&&actions[e]()}),$container.find(".prevented input").on("change",updateCurrentResult),$container.find(".comment textarea").on("change",updateCurrentResult).on("blur",updateCurrentResult),$container.find(".platform .value").html(resultData.platform),$container.find(".browser .value").html(resultData.browser),store("keycheck").then(function(storage){(keycheckStorage=storage).getItem("state").then(function(data){shortcutHelper.clear(),current=data.current||-1,shortcutsResults=data.results||{},0<=current&&current--,displaySection(
"shortcuts"),actions.nextShortcut()}).catch(function(){displaySection("shortcuts"),actions.resetAll()}),displaySection("shortcuts")}).catch(function(){displaySection("shortcuts"),actions.resetAll()}),binder.register("showShortcuts",actions.showShortcuts),binder.register("showResults",actions.showResults)}}}),define("taoDevTools/controller/routes",[],function(){return{ExtensionsManager:{actions:{index:"controller/settings/extensionManager"}},ScriptRunner:{actions:{index:"controller/ScriptRunner/index"}},FontConversion:{actions:{index:"controller/fontConversion/fileUpload"}},StudentTollGenerator:{actions:{index:"controller/fontConversion/fileUpload"}},
KeyChecker:{actions:{index:"controller/KeyChecker/index"}}}}),define("taoDevTools/controller/ScriptRunner/index",["jquery","helpers","ui/feedback"],function($,helpers,feedback){return{start:function(){$(".script").click(function(event){event=$(event.target).data("action");$.ajax({type:"POST",url:helpers._url(event,"ScriptRunner","taoDevTools"),dataType:"json",success:function(data){helpers.loaded(),data.success?feedback().success(data.message):feedback().error(data.message)}})})}}}),define("taoDevTools/controller/settings/extensionManager",["jquery","i18n","util/url","layout/section","ui/feedback","ui/modal"],function($,__,urlUtil,section,feedback){
var ext_installed=[],toInstall=[],indexCurrentToInstall=-1,percentByExt=0,installError=0;function progressConsole(msg){$("#installProgress .console").append("<p>"+msg+"</p>"),$("#installProgress .console").prop({scrollTop:$("#installProgress .console").prop("scrollHeight")})}function installNextExtension(){var ext=toInstall[indexCurrentToInstall];$("#installProgress p.status").text(__("Installing extension %s...").replace("%s",ext)),progressConsole(__("Installing extension %s...").replace("%s",ext)),$.ajax({type:"POST",url:urlUtil.route("install","ExtensionsManager","tao"),data:"id="+ext,dataType:"json",success:function(data){data.success?(progressConsole(
__("> Extension %s succesfully installed.").replace("%s",ext)),$("#installProgress .bar").animate({width:"+="+percentByExt+"%"},1e3,function(){++indexCurrentToInstall>=toInstall.length?(toInstall=[],$("#installProgress .bar").animate({backgroundColor:"#bb6",width:"100%"},1e3),progressConsole(__("Generating cache...")),$.ajax({type:"GET",url:$($("#main-menu a")[0]).prop("href"),success:function(data){$("#installProgress .bar").animate({backgroundColor:"#6b6"},1e3),$("#installProgress p.status").text(__("Installation done.")),progressConsole(__("> Installation done.")),window.location.reload()}})):installNextExtension()})):(installError=1,progressConsole(
"Installation of "+ext+" failed")),feedback().info(data.message)}}),installError&&progressConsole(__("A fatal error occured during the installation process."))}return{start:function(){$("#Extensions_manager table tr").removeClass("extensionOdd").removeClass("extensionEven"),$("#Extensions_manager table tr:nth-child(even)").addClass("extensionEven"),$("#Extensions_manager table tr:nth-child(odd)").addClass("extensionOdd"),$("#installProgress").hide(),$("#addButton").click(function(e){e.preventDefault(),section.create({id:"devtools-newextension",name:__("Create new extension"),url:urlUtil.route("create","ExtensionsManager","taoDevTools"),contentBlock:!0}
).show()}),$("#extensions-manager-container .ext-id").each(function(){var ext=$(this).text();ext_installed.push(ext),$(".ext-id.ext-"+ext).addClass("installed")}),$("#available-extensions-container tr input").click(function(event){event.stopPropagation()}),$("#available-extensions-container tr input:checkbox").click(function(){var $installButton=$("#installButton");0<$(this).parent().parent().parent().find("input:checkbox:checked").length?$installButton.attr("disabled",!1):$installButton.attr("disabled",!0)}),$("#installButton").click(function(event){var $modalContainer=$("#installProgress");if(event.preventDefault(),toInstall=[],$(
"#available-extensions-container input:checked").each(function(){var ext=$(this).prop("name").split("_")[1],deps=function getDependencies(extension){var dependencies=[];return $("#"+extension+" .dependencies li:not(.installed)").each(function(){var ext=$(this).attr("rel"),deps=getDependencies(ext);deps.push(ext),dependencies=dependencies.concat(deps)}),dependencies}(ext);(toInstall=deps.length?toInstall.concat(deps):toInstall).push(ext)}),0==(toInstall=function(orig){for(var a=[],i=0;i<orig.length;i++)$.inArray(orig[i],a)<0&&a.push(orig[i]);return a}(toInstall)).length)return window.alert(__("Nothing to install !")),!1;percentByExt=100/toInstall.length,$(
"p.status",$modalContainer).text(__("%s extension(s) to install.").replace("%s",toInstall.length)),$(".bar",$modalContainer).width(0),$(".console",$modalContainer).empty(),progressConsole(__("Do you wish to install the following extension(s):\n%s?").replace("%s",toInstall.join(", "))),$("[data-control=cancel]",$modalContainer).on("click",function(e){e.preventDefault(),$modalContainer.modal("close")}),$("[data-control=confirm]",$modalContainer).on("click",function(e){e.preventDefault(),progressConsole(__("Preparing installation...")),$(".buttons",$modalContainer).remove(),indexCurrentToInstall=installError=0,installNextExtension()}),$modalContainer.modal({
width:400,height:300,top:150,disableEscape:!0,disableClosing:!0})}),$(".disableButton").click(function(event){event=$(event.target).data("extid");$.ajax({type:"POST",url:urlUtil.route("disable","ExtensionsManager","taoDevTools"),data:"id="+event,dataType:"json",success:function(data){data.success?(feedback().success(data.message),setTimeout(function(){window.location.reload()},1e3)):feedback().info(data.message)}})}),$(".enableButton").click(function(event){event=$(event.target).data("extid");$.ajax({type:"POST",url:urlUtil.route("enable","ExtensionsManager","taoDevTools"),data:"id="+event,dataType:"json",success:function(data){data.success?(feedback(
).success(data.message),setTimeout(function(){window.location.reload()},1e3)):feedback().info(data.message)}})}),$(".uninstallButton").click(function(event){event=$(event.target).data("extid");$.ajax({type:"POST",url:urlUtil.route("uninstall","ExtensionsManager","taoDevTools"),data:"id="+event,dataType:"json",success:function(data){data.success?(feedback().success(data.message),setTimeout(function(){window.location.reload()},1e3)):feedback().info(data.message)}})})}}}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}(
